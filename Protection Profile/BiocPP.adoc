...
- [#CC-E&I]#[CC-E&I]# Errata and Interpretation for CC:2022 (Release 1) and CEM:2022 (Release 1), 001, Version 1.2, October 15, 2025.
- [#PP_MDF]#[PP_MDF]# Protection Profile for Mobile Device Fundamentals, May 1, 2026, Version 4.0.
- [#CFG-MDF-BIO]#[CFG-MDF-BIO]# PP-Configuration for Protection Profile for Mobile Device Fundamentals and collaborative PP-Module for Biometric enrolment and verification - for unlocking the device - [CFG-MDF-BIO], {revdate}, Version {revnumber}.
- [#TS 103 732-1]#[TS 103 732-1]# ETSI CYBER; Consumer Mobile Device; Part 1: Base Protection Profile, July 16, 2026, Version 3.1.1
- [#BIOSD]#[BIOSD]# Supporting Document Mandatory Technical Document: Evaluation Activities for collaborative PP-Module for Biometric enrolment and verification - for unlocking the device - [BIOSD], {revdate}, Version {revnumber}.
- [#ISO/IEC 19795-1]#[ISO/IEC 19795-1:2021]# Biometric performance testing and reporting — Part 1: Principles and framework, Edition 2.
- [#ISO/IEC 29156]#[ISO/IEC 29156]# Information technology - Guidance for specifying performance requirements to meet security and usability needs in applications using biometrics, Edition 1.
- [#ISO/IEC 30107-1]#[ISO/IEC 30107-1:2023]# Biometric presentation attack detection - Part 1: Framework, Edition 2.
- [#NIST800-63B]#[NIST800-63B]# NIST Special Publication 800-63B-4, Digital Identity Guidelines Authentication and Lifecycle Management, July 2025.
...
|2.1
|<date>
|Updates to the Attack Potential Table and PP-Modules for AVA_VAN and support for PP_MDF_V4.0

|{revnumber}
|{revdate}
|Updates to integrate with ETSI TS 103 732-1 v3.1.1
...
=== Threats - Selection-based

[[T.Scripted_Attack]]T.Scripted_Attack::
An attacker may attempt to impersonate as a legitimate user without being enroled in the TOE. In order to perform the attack, the attacker utilizes public scripts for creating a presentation attack instrument.

The threat <<T.Scripted_Attack>> is defined as attacks which are scored as meeting Basic according to the Ratings of vulnerabilities and TOE resistance in the BIOSD.
...
===== FCS_CKM.6 Timing and event of cryptographic key destruction (TS 103 732-1)
...
*Application Note {counter:remark_count}*:: A user enroling to the biometric system will have been authenticated using a NBAF, specified in FIA_UAU.5.1. Where the enrolment adds or changes a biometric template and therefore constitutes a change to the BAF, re-authentication shall be performed in accordance with FIA_UAU.6.1.

===== FIA_MBE_EXT.2 Quality of biometric templates for biometric enrolment [[FIA_MBE_EXT.2]]

*FIA_MBE_EXT.2.1*:: The TSF shall only use biometric samples of sufficient quality for enrolment. Sufficiency of sample data shall be determined by measuring sample with [*selection*: 
+
--
* [[*assignment*: _quality metric standard_] using a threshold of [*assignment*: _quality metric threshold_]], 
* [*assignment*: _developer defined quality assessment method_]
+
].
--
...
*FIA_MBV_EXT.2.1*:: The TSF shall only use biometric samples of sufficient quality for verification. Sufficiency of sample data shall be determined by measuring sample with [*selection*: 
+
--
* [[*assignment*: _quality metric standard_] using a threshold of [*assignment*: _quality metric threshold_]], 
* [*assignment*: _developer defined quality assessment method_]
...
*FIA_PAD_EXT.1.1*:: The TSF shall provide the [*selection*: _eye, face, fingerprint, vein_] with [*selection, from available PAD levels*: _PAD-L3, PAD-L2, PAD-L1, no PAD_] detection as specified in FIA_MBE_EXT.3 during biometric enrolment and [*selection, from available PAD levels, equal or greater than enrolment*: _PAD-L3, PAD-L2, PAD-L1, no PAD_] detection as specified in FIA_MBV_EXT.3 during biometric verification.

*Application Note {counter:remark_count}*:: The BIO-iTC website ({iTC-website}) specifies the available PAD toolboxes for each modality.
...
|<<T.Casual_Attack>>
.4+|The threat of zero-effort impostor attempt and presentation attack with related OSPs are specific subsets of the T.PHYSICAL_ACCESS (i.e. impersonate the user authentication mechanisms) threat in the <<PP_MDF>>.
...
For any modality selected in _FIA_UAU.5.1_, the mobile device shall invoke the Biometric System to unlock the device under the conditions specified in _FIA_UAU.6.1/CREDENTIAL_ and _FIA_UAU.6.1/LOCKED_. Mobile device shall also authenticate the user following the rule specified in _FIA_UAU.5.2_.
...
====== Invocation of the Biometric System
For any modality selected in _FIA_UAU.5.1/Local_, the mobile device shall invoke the Biometric System to unlock the device under the conditions specified in _FIA_UAU.6.1_. Mobile device shall also authenticate the user following the rule specified in _FIA_UAU.5.2/Local_.
...
====== Management of the Biometric System configuration
The mobile device shall enable/disable/remove the BAF as required by _FMT_SMF.1/Authentication_. Any change to the BAF (e.g. adding or revoking templates) requires re-authentication via the Password Authentication Factor as required by _FIA_UAU.6.1_.
...
The PP-Module does not contain any optional requirements.
...
[appendix]
[[L2PP-Module]]
== collaborative PP-Module with PAD-L2 for Biometric verification [L2PP-Module]
=== Introduction
==== PP-Module Identification

* PP-Module Reference: collaborative PP-Module with PAD-L2 for Biometric verification [L2PP-Module]
* PP-Module Version: {revnumber}
* PP-Module Date: {revdate}

==== PP-Module Base identification
The L2PP-Module is intended for use with the following PP-Module:
collaborative PP-Module for Biometric enrolment and verification - for unlocking the device - [BIOPP-Module], version {revnumber}.

==== TOE Overview
===== TOE Main Security Features
The scope of the L2PP-Module is to describe the assurance requirements when PAD-L2 is selected as the level of protection against presentation attacks for the biometric system. There are no changes to the scope of the system, only to the claims of the level of protection made in terms of protecting against presentation attacks. In this case, an explicit claim of AVA_VAN.2 is made on this requirement.

When the PP-Module Base already includes AVA_VAN.2, this L2PP-Module should be included.

===== TOE Design
The TOE design is identical to the BIOPP-Module.

===== Relation Between TOE and Computer
The relation between the TOE and the computer is identical to the BIOPP-Module.

==== TOE Use Case
The L2PP-Module does not specifically define a use case, though it aligns with the Use Case 2: Biometric verification for security sensitive services.
...
The L2PP-Module does not define any organizational security policies.

==== Assumptions

The L2PP-Module does not define any assumptions.
...
==== BIOPP-Module Security Functional Requirements Direction

The SFRs listed in this section are defined in the <<Security Functional Requirements, PP-Module Base>> and are modified for use to conform to this L2PP-Module. If the computer supports multiple biometric modalities which do not all meet the requirements of this L2PP-Module, the SFRs shuld be iterated to clearly show which modalities support which PAD levels.

===== FIA_PAD_EXT.1 Presentation attack support

*FIA_PAD_EXT.1.1*:: The TSF shall provide the [*selection*: _face, fingerprint_] with [PAD-L2] detection as specified in FIA_MBE_EXT.3 during biometric enrolment and [*selection, from available PAD levels, equal or greater than enrolment*: PAD-L2] detection as specified in FIA_MBV_EXT.3 during biometric verification.

===== Additional SFRs

There are no additional SFRs that to be claimed specifically for this L2PP-Module.
...
This L2PP-Module is created following the multi-assurance evaluation paradigm that consists in applying different assurance requirements to different parts of the TSF (sub-TSFs), while enforcing a global set of SARs for the entire TOE. In this case, the entire TOE corresponds to computer containing the biometric system and the sub-TSF corresponds to the biometric system.

The L2PP-Module augments the SARs from the PP-Module Base with the addition of AVA_VAN.2 to counter against <<T.Intentional_Attack>> that assumes higher attack potential for presentation attacks. The TOE evaluation should follow multi-assurance evaluation paradigm when the SARs for the TOE (i.e., computer) includes lower component, AVA_VAN.1.

AVA_VAN.2 has additional dependencies on ADV_ARC.1, ADV_FSP.2 and ADV_TDS.1 in addition to those components depended by AVA_VAN.1, however, those dependencies are removed in this L2PP-Module. The focus of adding AVA_VAN.2 for FIA_PAD_EXT.1 is in increasing the level of presentation attacks, not in bypass or injection attacks, of the whole system. As such, zero- or limited-knowledge attacks are what is expected, the attacks focused on actually attempting to bypass the system with a fake, rather than by attacking the underlying system design itself.

The requirements in the PAD-L2 toolboxes are designed to specifically test higher quality species to determine if the system is able to be bypassed, regardless of the design. The attack scenarios or recipes require Enhanced-Basic attack potential are defined in the modality Toolbox as actual direct attacks independent of design information, to verify that the TOE can counter against <<T.Intentional_Attack>>. 

Depending on the PP-Module Base for the BIOPP-Module it is possible that AVA_VAN.2 is already included in the set of SARs for the computer containing the biometric system. In this case the SAR for the overall TOE does not change but the evaluation activities related to the selected PAD component are decribed in the BIOSD specifically as it ties to AVA_VAN.2.

=== Consistency Rationale

This section describes consistency rationale between <<BIOPP-Module>> and the L2PP-Module to show that the unions of Security Problem Definition, objectives, and Security Functional Requirements defined in <<BIOPP-Module>> and the L2PP-Module do not lead to a contradiction.

==== Consistency of Security Problem Definition

The threat added in the L2PP-Module is a replacement for an existing threat in the PP-Module Base with a higher attack potential rating.

<<T.Intentional_Attack>> encompasses <<T.Scripted_Attack>> and so follows the same consistency rationale when applied to the PP-Module Base of the BIOPP-Module. 

==== Consistency of Objectives

The L2PP-Module does not add any objectives to the TOE.

==== Consistency of Requirements

The L2PP-Module restricts the selections in FIA_PAD_EXT.1 to be limited solely to biometric modalities that have PAD-L2 toolboxes for testing. 

[appendix]
== collaborative PP-Module with PAD-L3 for Biometric verification [L3PP-Module]
=== Introduction
==== PP-Module Identification

* PP-Module Reference: collaborative PP-Module with PAD-L3 for Biometric verification [L3PP-Module]
* PP-Module Version: {revnumber}
* PP-Module Date: {revdate}

==== PP-Module Base identification
The L3PP-Module is intended for use with the following PP-Module:
collaborative PP-Module for Biometric enrolment and verification - for unlocking the device - [BIOPP-Module], version {revnumber}.

==== TOE Overview
===== TOE Main Security Features
The scope of the L3PP-Module is to describe the assurance requirements when PAD-L3 is selected as the level of protection against presentation attacks for the biometric system. There are no changes to the scope of the system, only to the claims of the level of protection made in terms of protecting against presentation attacks. In this case, an explicit claim of AVA_VAN.3 is made on this requirement.

When the PP-Module Base already includes AVA_VAN.3, this L3PP-Module should be included.
...
==== TOE Use Case
The L3PP-Module does not specifically define a use case, though it aligns with the Use Case 2: Biometric verification for security sensitive services.
...
The L3PP-Module does not define any organizational security policies.

==== Assumptions

The L3PP-Module does not define any assumptions.
...
The SFRs listed in this section are defined in the <<Security Functional Requirements, PP-Module Base>> and are modified for use to conform to this L3PP-Module. If the computer supports multiple biometric modalities which do not all meet the requirements of this L3PP-Module, the SFRs shuld be iterated to clearly show which modalities support which PAD levels.

===== FIA_PAD_EXT.1 Presentation attack support

*FIA_PAD_EXT.1.1*:: The TSF shall provide the [*selection*: _face, fingerprint_] with [PAD-L3] detection as specified in FIA_MBE_EXT.3 during biometric enrolment and [*selection, from available PAD levels, equal or greater than enrolment*: PAD-L3] detection as specified in FIA_MBV_EXT.3 during biometric verification.

===== Additional SFRs

There are no additional SFRs that to be claimed specifically for this L3PP-Module.

==== TOE Security Functional Requirements

The L3PP-Module does not define any additional SFRs.
...
This L3PP-Module is created following the multi-assurance evaluation paradigm that consists in applying different assurance requirements to different parts of the TSF (sub-TSFs), while enforcing a global set of SARs for the entire TOE. In this case, the entire TOE corresponds to computer containing the biometric system and the sub-TSF corresponds to the biometric system.

The L3PP-Module augments the SARs from the PP-Module Base with the addition of AVA_VAN.3 to counter against <<T.Targeted_Attack>> that assumes higher attack potential for presentation attacks. The TOE evaluation should follow multi-assurance evaluation paradigm when the SARs for the TOE (i.e., computer) includes lower component, AVA_VAN.1.

AVA_VAN.3 has additional dependencies on ADV_ARC.1, ADV_FSP.4, ADV_TDS.3, ADV_IMP.1 and ATE_DPT.1 in addition to those components depended by AVA_VAN.1, however, those dependencies are removed in this L3PP-Module. The focus of adding AVA_VAN.3 for FIA_PAD_EXT.1 is in increasing the level of presentation attacks, not in bypass or injection attacks, of the whole system. As such, zero- or limited-knowledge attacks are what is expected, the attacks focused on actually attempting to bypass the system with a fake, rather than by attacking the underlying system design itself.

The requirements in the PAD-L3 toolboxes are designed to specifically test higher quality species to determine if the system is able to be bypassed, regardless of the design. The attack scenarios or recipes require Medium attack potential are defined in the modality Toolbox as actual direct attacks independent of design information, to verify that the TOE can counter against <<T.Targeted_Attack>>. 

Depending on the PP-Module Base for the BIOPP-Module it is possible that AVA_VAN.3 is already included in the set of SARs for the computer containing the biometric system. In this case the SAR for the overall TOE does not change but the evaluation activities related to the selected PAD component are decribed in the BIOSD specifically as it ties to AVA_VAN.3.

=== Consistency Rationale

This section describes consistency rationale between <<BIOPP-Module>> and the L3PP-Module to show that the unions of Security Problem Definition, objectives, and Security Functional Requirements defined in <<BIOPP-Module>> and the L3PP-Module do not lead to a contradiction.

==== Consistency of Security Problem Definition

The threat added in the L3PP-Module is a replacement for an existing threat in the PP-Module Base with a higher attack potential rating.

<<T.Intentional_Attack>> encompasses <<T.Scripted_Attack>> and so follows the same consistency rationale when applied to the PP-Module Base of the BIOPP-Module. 

==== Consistency of Objectives

The L3PP-Module does not add any objectives to the TOE.

==== Consistency of Requirements

The L3PP-Module restricts the selections in FIA_PAD_EXT.1 to be limited solely to biometric modalities that have PAD-L3 toolboxes for testing. 
