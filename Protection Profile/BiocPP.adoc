...
The L2PP-Module augments the SARs from the PP-Module Base with the addition of AVA_VAN.2 Vulnerability Analysis. The AVA_VAN.2 SAR added here is focsued on the SFRs included in the L2PP-Module based on the inclusion of the <<T.Intentional_Attack>>. This Threat is designed to represent attacks with a higher Attack Potential as defined in the BIOSD in Section 11 that matches with AVA_VAN.2. This SAR does not impact the overall AVA_VAN level for the TOE, only for the SFRs included in the L2PP-Module.
...
The L3PP-Module augments the SARs from the PP-Module Base with the addition of AVA_VAN.3 Focused Vulnerability Analysis. The AVA_VAN.3 SAR added here is focsued on the SFRs included in the L3PP-Module based on the inclusion of the <<T.Targeted_Attack>>. This Threat is designed to represent attacks with a higher Attack Potential as defined in the BIOSD in Section 11 that matches with AVA_VAN.3. This SAR does not impact the overall AVA_VAN level for the TOE, only for the SFRs included in the L3PP-Module.
