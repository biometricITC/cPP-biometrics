...
* PP-Module Reference: PAD-L2 Module for the BIOPP-Module
* PP-Module Version: {revnumber}
* PP-Module Date: {revdate}

==== PP-Module Base identification
This PP-Module is intended for use with the following PP-Module:
collaborative PP-Module for Biometric enrolment and verification - for unlocking the device - [BIOPP-Module], version {revnumber}.

==== TOE Overview
===== TOE Main Security Features
The scope of this PP-Module is to describe the assurance requirements when PAD-L2 is selected as the level of protection against presentation attacks for the biometric system. There are no changes to the scope of the system, only to the claims of the level of protection made in terms of protecting against presentation attacks. In this case, an explicit claim of AVA_VAN.2 is made on this requirement.

When the PP-Module Base already includes AVA_VAN.2, this PP-Module should be included.
...
==== TOE Use Case
This PP-Module does not specifically define a use case, though it aligns with the Use Case 2: Biometric verification for security sensitive services.
...
* PP-Module Reference: PAD-L3 Module for the BIOPP-Module
* PP-Module Version: {revnumber}
* PP-Module Date: {revdate}
...
==== TOE Overview
===== TOE Main Security Features
The scope of this PP-Module is to describe the assurance requirements when PAD-L3 is selected as the level of protection against presentation attacks for the biometric system. There are no changes to the scope of the system, only to the claims of the level of protection made in terms of protecting against presentation attacks. In this case, an explicit claim of AVA_VAN.3 is made on this requirement.

When the PP-Module Base already includes AVA_VAN.3, this PP-Module should be included.
