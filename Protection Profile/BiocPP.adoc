...
AVA_VAN.2 has additional dependencies on ADV_ARC.1, ADV_FSP.2 and ADV_TDS.1 in addition to those components depended by AVA_VAN.1, however, those dependencies are removed in this L2PP-Module because attack scenarios or recipes that require Enhanced-Basic attack potential are defined in Toolbox and the evaluator doesn't need such design information to verify that the TOE can counter against <<T.Intentional_Attack>>.
