...
[NOTE]
====
TODO: Need to add additional EAs for modified SFR to <<BIOSD>>
====
...
*FPT_KST_EXT.2.1*:: The TSF shall not transmit any plaintext key material *and biometric data* outside the security boundary of the TOE.
...
This PP-Module does not modify SFRs in FIA class as it is defined in the <<PP_MD_V3.3>>. However, if the TOE claims conformance to a PP-Configuration that includes this PP-Module, only eye, face, fingerprint or vein must be selected in the <<PP_MD_V3.3>> and the same modality must also be selected in this PP-Module.
