...
* Base-PP: Protection Profile for Mobile Device Fundamentals, Version:3.3. <<PP_MD>>
...
This PP-Configuration, <<PP_MD>> and <<BIOPP-Module>> are conformant to Common Criteria Version 3.1, Revision 5.
...
In order to evaluate a TOE that claims conformance to this PP-Configuration, the evaluator shall evaluate the TOE against the following SARs that are defined in the <<PP_MD>>:
...
Note that to fully evaluate the TOE, these SARs shall be applied to the entire TSF and not just the portions described by <<PP_MD>> where the SARs are defined.
