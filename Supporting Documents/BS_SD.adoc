...
. Quality standard that the TOE uses to perform the assessment if the TOE follows such standard (e.g. <<NFIQ2>> for fingerprint)
...
=== Summary of contents
<<QualityItemsTable>> shows items that shall be reported in the performance report. The name or structure of quality assessment criteria report does not need to follow <<QualityItemsTable>>. However, all items in <<QualityItemsTable>> shall be included somewhere in the report. Also, if some items are not included in the report, the developer shall provide a rationale for such exclusion to the evaluator.
...
The developer may point to a public standard used for the determining of qualtiy criteria, such as <<NFIQ2>> or <<ISO29794-1, ISO/IEC 29794-1>> to provide the necesasry information. The developer must provide information about how the public standard is used by the TOE.
...
The discussion of the algorithms(s) should include information about the defects that are considered and handled in determining quality. The specific algoirthm(s) used to generate a qualtiy score, whether general or modality-specific, must be covered. 
...
- [#NFIQ2]#[NFIQ2]# NIST Fingerprint Image Quality 2, https://www.nist.gov/services-resources/software/nfiq-2.
