...
====== Strategy for ATE_IND
Any plaintext biometric data must not be accessible from any entities outside the SEE, especially the main computer operating system, to meets FPT_BDP_EXT.1. This means that a) the TOE must not expose the plain biometric data to the memory that is accessible by the operating system during the processing of biometric data and b) any TSFIs that can output the plain biometric data must not be existed or accessible by the operating system.

The evaluator shall perform the following tests to verify that both a) and b) described above are true.

If it is impractical or inadequate to conduct the following tests, the developer may propose alternate approach to verify a) and b). It is the evaluator's responsibility to determine the suitability of the alternate approach. However, for example, an analysis of source codes to determine that a) and b) are met by the TOE is an acceptable alternate approach, as described in <<CEM>>.
...
+
[arabic]
.. The evaluator shall identify any TSFIs that output the plaintext biometric data to the memory that is accessible by the operating system. The evaluator shall especially examine TSFIs of TSF modules provided by the biometric capture sensor (e.g. SDK) because they may include testing or debug codes and the developer who integrated the sensor into the TOE may apply minimal changes to those modules
+
If the evaluator can’t find such TSFIs, the following test can be skipped. 
.. The evaluator shall conform that invocation of the TSFIs by the operating system are failed.
...
===== Objective of the EA
Only an authenticated user can add one's own templates during biometric enrolment as defined in the FIA_MBE_EXT.1 and those templates are not stored as plaintext as required by the modified FPT_KST_EXT.1 defined in <<BIOPP-Module>>. However, the TOE may provide functions (e.g. revocation of templates) to access the templates. The evaluator shall confirm that only an authenticated user using a NBAF as specified by the ST author can access the templates through the TSFI provided by the TOE.
