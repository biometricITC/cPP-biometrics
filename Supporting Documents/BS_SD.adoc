...
. The evaluator shall review public papers on biometric quality (see <<Related Documents>> for some referenced quality papers) for the appropriate modality to determine how to create low quality samples. The evaluator shall document the papers used for reference in the evaluation report.
.. The evaluator may ask the developer for assistance if public methods are unsuccessful in creating the low quality samples
