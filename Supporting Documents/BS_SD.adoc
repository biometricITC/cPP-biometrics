...
The evaluator shall follow the same EAs defined in <<PP_MD_V3.3>> and BIOSD. As explained in <<Evaluation Activities for PAD testing>>, detail of PAD testing for ATE_IND.1 is defined in <<Toolbox>>.
...
The evaluator shall follow the same EAs defined in <<PP_MD_V3.3>> and BIOSD. As explained in <<Evaluation Activities for PAD testing>>, detail of PAD testing for AVA_VAN.1 is defined in <<Toolbox>>.
...
The developer needs to assign the maximum IAPAR in FIA_MBV_EXT.3. The evaluator shall follow the information in <<Toolbox>>, for example, the number of artefact presentations, to conduct the testing and confirm that measured IAPARs for all artefacts are equal to or less than the assigned IAPAR through the independent testing. 
...
The <<Toolbox>> describes a test subset and test documentation that is sufficiently detailed to enable the tests to be reproducible (ATE_IND.1-3 and ATE_IND.1-4). The <<Toolbox>> also includes information that support evaluator’s decision (ATE_IND.1-7).
...
The evaluator shall assign a fail verdict if the evaluator can find the artefact of which IAPAR is higher than the maximum IAPAR and it can be reproduced reliably and repeatably by an attacker possessing a Basic attack potential.
