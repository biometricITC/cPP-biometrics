...
+
If TSFIs exist the can be used to output plaintext biometric data to the operating system, the evaluator must perform this test. The following test steps require the developer to provide access to a test platform that provides the evaluator with tools that are typically not found on factory products.
+
[arabic]
