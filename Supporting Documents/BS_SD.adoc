...
==== EA for FTP_TRP.1
===== Objective of the EA
The path between the biometric capture sensor and the SEE where processing of the biometric characteristics will be performed per <<FPT_BDP_EXT.1>> or interactions to the main computer operating system occur (such as providing the results of a match), must protect the captured data from being tampered with or duplicated in transit. The evaluator shall confirm that this data path is protected from access by untrusted components (generally defined as anything in the main computer operating system). 
...
[loweralpha]
. TSS shall explain how the TOE meets FTP_TRP.1 at high level description
. BMD shall provide additional information needed to explain the protection mechanism within the scope of the assurance level claimed by <<BIOPP-Module>>

===== Assessment Strategy
====== Strategy for ASE_TSS and AGD_OPE/ADV_FSP
The evaluator shall examine the TSS and BMD to verify that all TSF modules (i.e. biometric capture sensor and any software running in biometric capture sensor and the computer processors) must send data via paths that are not accessible from any entities outside the SEE and the components used in the capture process.

In any case, the evaluator shall examine the TSS and BMD to confirm that;

[loweralpha]
. All TSF modules use a trusted path to transmit any data between components and the SEE and any entities outside the SEE including the main computer operating system can’t interfere with processing of these modules

The evaluator shall keep in mind that the objective of this EA is not evaluating the SEE itself. This EA is derived from ASE_TSS.1.1 which requires that the TSS to provide potential consumers of the TOE with a high-level view of how the developer intends to satisfy each SFR. The evaluator shall check the TSS and BMD to seek for a logical explanation how the above criteria are satisfied considering this scope of the requirement.
...
. The TOE protects the biometric data captured while it is being trnasferred to the SEE
