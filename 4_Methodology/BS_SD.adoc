...
. Only a user authenticated by password can enrol him/herself and any attempts to enrol without the authentication are rejected through the independent testing
...
. Supplementary information (developer’s [#performance report]#performance report#) shall describe developer’s performance test protocol and result of testing
...
<<Toolbox>> is intended to be a state of the art set of test and is continually updated. The latest versions of the <<Toolbox>> for each supported modality must be used by the evaluator at the time the testing begins.
...
[arabic]
.. A user will enrol him/herself following guidance provided by the TOE
...
The evaluator shall test those test items created for new artefacts first, as described in <<New artefacts found test plan>>, following the same procedure as the one used for the independent testing.
The evaluator shall also update those new test items to create the “higher quality” artefacts as explained in <<No new artefacts found test plan>> based on result of this initial testing.
...
- [#Toolbox]#[Toolbox]# Toolbox Overview    
