...
The original focus for the iTC was to create a standalone PP that would be evaluated separately but recommended to be used with the other PP (e.g. MDFPP or GPOSPP) that defines security requirements to provide the secure environment for the biometric enrolment and verification. Later the iTC modified the PP into a PP-Module for somewhat easier integration with the MDFPP (the primary target device at this time).
